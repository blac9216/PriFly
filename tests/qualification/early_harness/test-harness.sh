...
# directory is lowercase hex under TMPDIR and TMPDIR itself must hold no upper case.
set -euo pipefail
...
mkdir -m 700 "$work"
cleanup() {  # kill only writer pids this test's fake launcher recorded, if still ours
  set +e
  for f in "$work"/*/state/*.pids; do
    while read -r p; do grep -qs sentinel "/proc/$p/cmdline" && kill "$p"; done <"$f"
  done
...
survivors() { cat "$work/$1"/state/*.pids | while read -r p; do grep -qs sentinel "/proc/$p/cmdline" && echo "$p"; done | wc -l; }
...
PY
  set +e; PATH="${PROBE_PATH:-$PATH}" FAKE_MODE="$2" FAKE_STATE="$d/state" bash "$HARNESS" --manifest "$d/manifest.json" \
...
check "mid-run stop failure left no writer" 0 "$(survivors mid-run-stop-failure)" "" /dev/null
probe "signal during a run" signal 300 21 "ABORTED: exit 143 after launch began; no verdict"
check "signal left no writer" 0 "$(survivors signal-during-a-run)" "" /dev/null
...
for ms in 0300 3e2 1000000 ""; do
  probe "observe-ms '$ms'" ok "$ms" 20 "REFUSED: --observe-ms must be 0..999999 without a leading zero; no probe step ran"
