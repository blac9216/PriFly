...
PY
  set +e; PATH="${PROBE_PATH:-$PATH}" LC_ALL="${PROBE_LC:-${LC_ALL:-}}" FAKE_MODE="$2" FAKE_STATE="$d/state" bash "$HARNESS" --manifest "$d/manifest.json" \
...
for sig in term int hup twice; do  # lower case: upper case in the work path trips #240
  ...
  check "signal $sig left no writer or launcher" "0 0" "$(reap "$d" '*.pids' sentinel) $(reap "$d" launchers fake-launcher)" "" /dev/null
done
