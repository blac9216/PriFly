...
### Initial container-owned attempt profile

A trusted launcher attaches a disposable attempt container to a controller-owned HerdR pane. Only the launcher can control attempt lifecycle through the stack engine; its socket is unavailable to Workers. Jobs use a separate Worker Docker daemon through an attempt-scoped proxy, with enforced resource identity, mount/port/privilege constraints and an explicit resource inventory. Worker Docker access cannot create or alter Factory attempts. A single-use published run claim precedes harness start. Lost launch responses reconcile the actual container/process and server/pane identity before any resend. No automatic restart, native resume or experimental handoff is admitted.

On cancellation or handoff, revoke the capability, stop the entire attempt process boundary, reconcile Docker writers and retained workspace services, and verify Git/dirty state before releasing the lease. Any surviving or unknown writer quarantines the workspace. Services that can modify the candidate tree must be paused/stopped even when otherwise workspace-owned; safe cache-only services may remain. Review uses a clean separate checkout/home and test resources. A fresh correction keeps the Work Item workspace but never inherits the producer conversation or auth files. [ADR-0030](../adr/0030-own-attempts-behind-herdr-launcher.md) records this topology and its trusted-engine trade-off.

Launch uses published CREATE_ARMED → observed immutable container ID → published START_ARMED(exact ID) before Engine Start. Cancellation closes further arm admission and retires every possibly-started exact container ID through verified removal before handoff. Delayed create can produce only a stopped restart-disabled container and cannot gain a new start permit. Delayed Start for the retired ID must fail under the qualified engine contract. Unknown removal preserves the lease/quarantine; stopped-only inspection does not fence an in-flight start. Recovery inherits these possible-send runtime obligations just as it inherits provider obligations.

Worker Docker proxy operations that can create/restart a workspace writer obey the same published exact-ID start-arm and verified retirement protocol as harness attempts. Delayed creates remain stopped and cannot gain new authority after cancellation; delayed starts address retired IDs and cannot revive writers. A raw Worker socket or unsupported Compose/restart bypass is inadmissible. Trusted Git separately measures a frozen code checkpoint (≤128 MiB uncompressed per snapshot, generated caches/build output excluded), with ≤1 GiB new checkpoint data per handover trial; unknown pushes retain reservation. R2 accounting does not pretend to include another provider's stored bytes.
