...
The bounded R2 transport is part of the trusted Factory boundary. Internal service authentication cannot authorize arbitrary prefixes/operations, Workers have no direct R2 credentials, and a replaced transport incarnation cannot reset an old grant. Its forwarding/header/identity rules and pre-send debit are independent conformance obligations, not a claim that an S3-compatible proxy is transparent by default.
