...
| Workflow/documentation agreement | `bash scripts/docs/check-ci-agreement.sh --root .` | Repository checkout; Bash, Python 3. Fails unless every step of `docs-checks.yml` and `go-checks.yml` pairs, in order, with its Commands-table row here or appears on the checker's own written exemption list, and unless `doc-manifest.md`'s `## CI` list and its spelled-out count match this job's steps. |
...
step or row that is neither paired nor on its written exemption list. The checker's
header records that list and the three classes of normalisation it applies between a
table cell and a `run:` line, and it fails rather than skipping when it meets a cell or
a step it cannot normalise.
