...
| Harness proof | `bash tests/qualification/early_harness/test-harness.sh` | Same; `python3`, `setsid`, `ps`, `timeout` from GNU coreutils or uutils coreutils (the runner kills an expired call's process group itself, so the proof passes with either first on `PATH`), the `en_US.UTF-8` locale, and the namespaces above; `TMPDIR` with no upper case (#240). Prints one `ok` line per check and `failures: 0`. |
