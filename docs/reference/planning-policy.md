...
## External package gate equivalence

The initial execution profile may import independently evaluated external artifacts through an owner-registered provenance trust profile. The same exact-criterion/applicability, coverage, blocking Findings, graph and owner-release rules apply. Import itself grants no phase authority, and owner provenance attestation cannot overwrite failing engineering evidence. Unknown historical evidence is represented honestly rather than synthesized. Native planning/triage jobs remain unsupported until admitted; external change/follow-up packages preserve all canonical obligations. See [ADR-0029](../adr/0029-admit-external-reviewed-execution-packages.md).
