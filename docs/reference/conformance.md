...
Required additional profile observations: (1) externally imported stale/forged/incomplete packages never dispatch; (2) real interactive Codex and Claude attempts cannot access controller sockets/other workspaces and all sentinels/Docker writers stop before handoff; (3) every publication's exact remote restore contains the expected sequence/command and no pending successor; (4) retention/compaction/local loss preserves current and older supported roots while unsafe lineage reuse/reset is rejected; (5) actual App/rules/check identities enforce qualified head/base behavior and unknown sends retain conflicts; (6) versioned validation and external corrective planning preserve every defect obligation; (7) old/new controller lost-reply trials enforce the publication rollback cutoff; (8) fixed-load/resource/storage pressure preserves control-plane headroom and never deletes required evidence. These are required future tests, not claimed PASS evidence.

The handover proof contains two meaningful code Work Items with overlapping progress across both harnesses, fresh independent review/current correction, Factory-driven integration, intended-use validation, remote-only host-loss recovery, and activation of the validated candidate controller. Prewritten patches, terminal completion, manual merges or undocumented repairs cannot satisfy that proof.

Adversarial lifecycle qualification delays Create/Start across cancellation, retirement, successor lease and takeover. A stopped-only control must fail; retired IDs cannot write. Storage qualification exhausts permits and loses debit/response continuity across incarnation changes: no unsafe reuse/refill, unbounded body or under-counted old sender is permitted. Fixed-load tests use the selected seeded query/command mix and per-run percentile method, including queue delay and every failure sample.
