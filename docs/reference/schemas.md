...
Launch binding includes generation, attempt/run, immutable prompt/command digest, server/pane incarnation and container/process inventory. Provider obligations include stable operation/payload/profile/identity, authorization source, send state, reconciliation and conflict reservation. A publication verification receipt records exact lineage/TXID/application sequence/command/tool/config and invariant evidence and is externally referenced by coordination without a recursive DB write. Executable schemas validate structure; Go checks semantic cross-record/authority invariants before any admission.

Runtime possible-send records identify CREATE_ARMED/CREATE_OBSERVED/START_ARMED and exact engine/container identity. Storage grants identify generation, adapter incarnation, prefix/operation set, deadline and byte/object/request maxima; unresolved grants remain fully reserved. Local pre-send debit loss is conservatively charged, not treated as unused budget.
