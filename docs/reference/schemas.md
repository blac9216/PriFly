...
Launch binding includes generation, attempt/run, immutable prompt/command digest, server/pane incarnation and container/process inventory. A publication verification receipt records exact lineage/TXID/application sequence/command/tool/config and invariant evidence and is externally referenced by coordination without a recursive DB write.
