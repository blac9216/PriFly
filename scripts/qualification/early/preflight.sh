...
# a secret-looking key prints as <redacted-key>. Exit codes: 0 every reference named (not a
# PASS), or usage when -h/--help is the sole argument; 2 usage error (-h/--help beside any
# other argument included) or unreadable/non-JSON manifest; 3 schema file missing or using
# an unsupported keyword; 4 manifest invalid (shape, tuple count, secret-looking value or
...
SECRET = re.compile(r'-----BEGIN|AGE-SECRET-KEY-|\b[sr]k-[A-Za-z0-9]|\bgh[pousr]_|github_pat_|\bxox[abpr]-|\bAKIA[0-9A-Z]{12}|\beyJ[A-Za-z0-9_-]{8}')
invalid, holds = [], []
...
def check(s, v, path, out):
    ...
    # ECMA-262 '$' never matches before a trailing newline; Python's does, so end on \Z.
    if 'pattern' in s and isinstance(v, str) and not re.search(re.sub(r'\$$', r'\\Z', s['pattern']), v):
...
def looks(x):
    return bool(SECRET.search(x) or any(
        all(re.search(c, r) for c in ('[a-z]', '[A-Z]', '[0-9]')) for r in re.findall(r'[A-Za-z0-9+=]{32,}', x)))

def seg(path, k):
    return f"{path}.{'<redacted-key>' if looks(k) else k}".lstrip('.')
