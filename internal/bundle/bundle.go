...
//
// Which diagnostics the cap keeps is decided by path order and by nothing else.
...
// Neither the code, nor how many findings a code already has, nor how serious
// one is enters that decision, and a bundle dense enough to reach the bound can
// therefore have the only finding of one code cut while a thousand of another
// are listed. The trigger is a total over every code together, not a quota per
// code. done says the list was cut, with incomplete, but not what left it.
//
...
//
// TestCheckerBudgetBuysSmallestPaths and TestCheckerBudgetIsATotalOverCodes pin
// both halves, and TestCheckerKeepsFirst pins that none of it depends on the
// order the diagnostics were added in.
